...
from CUB.utils import IndexToString
    ...
    for data_idx, data in enumerate(loader):
        ...
        if args.use_attr:
            if args.no_img:  # A -> Y
                ...
                img_paths = list(img_path)
                for i in range(attr_outputs_sigmoid.size()[0]):
                    attr_outputs_labels = attr_outputs_sigmoid[i]
                    attr_outputs_labels = torch.sigmoid(attr_outputs_labels).data.cpu().numpy()

                    class_outputs_index = class_outputs.argmax(dim=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-model_dirs2', default=None, nargs='+',
    ...
    parser.add_argument('-attribute_group', default=None,
